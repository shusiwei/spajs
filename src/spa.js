...
;(function(global, factory, template) {
  ...
  if (typeof define === 'function' && define.amd) {
    define(['jquery', './doT'], function($, doT) {
      return factory(global, $, doT);
    });
  } else if (typeof define === 'function' && define.cmd) {
    define(function(require, exports, module) {
      module.exports = factory(global, require('jquery'), require('./doT'));
    });
  } else {
    global.SPA = factory(global, global.jQuery, global.doT);
  };
  ...
  class SPA {
    constructor(project, options = {}) {
      ...
      let HASH_PREFIX = options.prefix || '#!',
        ...
        // 项目相关信息
        PROJECT_NAME = options.name || this.project,
        PROJECT_VER = options.version,
        PROJECT_DIR = options.root ? options.root : './project/',

        // 页面请求相关配置
        API_TYPE = options.apiType || 'post',
        API_DATA_TYPE = options.apiDataType || 'json',
        API_DATA = options.apiData || {},
        API_DATA_KEY = options.apiDataKey || 'data',
        API_MSG_KEY = options.apiMsgKey || 'msg',
        API_CODE_KEY = options.apiCodeKey || 'status',
        TIME_OUT = options.timeout || 15e3,

        // 页面资源相关配置
        IMG_DIR = PROJECT_DIR + (options.imgDir || 'img'),
        JS_DIR = PROJECT_DIR + (options.jsDir || 'js'),
        TPL_DIR = PROJECT_DIR + (options.tplDir || 'tpl'),
        TPL_EXT_NAME = options.debug === true ? '.tpl' : '-' + PROJECT_VER + '.tpl',
        JS_EXT_NAME = options.debug === true ? '.js' : '-' + PROJECT_VER + '.js',

        LANGUAGE = options.language,

        // 默认主页的HASH
        HOME_HASH = HASH_PREFIX + (options.homeHash || 'index'),

        // 页面自动销毁的时间
        AUTO_DIE_TIME = options.lifeCycle || 90e3;
    ...
    start(container, storage = {}, callback = {}) {
      // 全局对象
      let activeView = null;
      const [document, location, history, sessionStorage] = [global.document, global.location, global.history, global.sessionStorage],
        [project, routes] = [this.project, this.routes],
        {PROJECT_NAME, PROJECT_DIR, PROJECT_VER, HASH_PREFIX, HASH_LENGTH, API_TYPE, API_DATA_TYPE, API_DATA, API_DATA_KEY, API_MSG_KEY, API_CODE_KEY, IMG_DIR, JS_DIR, TPL_DIR, TPL_EXT_NAME, JS_EXT_NAME, HOME_HASH, TIME_OUT, AUTO_DIE_TIME, LANGUAGE} = this.config,
      ...
      class WebApp {
        ...
        static parseHash(hash) {
          ...
          if (isUndefined(tempRoute)) {
            ...
            try {
              rules.lang = ['descriptor', LANGUAGE[routeName], {
                enumerable: true
              }];
            } catch (e) {
              rules.lang = null;
            ...
            rules.lang = ['descriptor', tempRoute.lang, {
              enumerable: true
            }];
          };
        ...
        static getWebView(hash, source, data) {
          ...
          if (!cache[path].hasOwnProperty(search)) {
            ...
            // 如果此webview已经被注销
            if (webview.prop.status === 'destroy') {
              // 重新创建一个webview容器并更新
              webview.create(data);
            // 如果此页状态还未被注销，并且此页来源于链接
            } else if (source === 1) {
        ...
        request(hash, state, source, data) {
          ...
          if (WebApp.hasTpl(webview.path)) {
            ...
            // 请求页面数据
            if (!isNull(webview.route.uri)) webview.runtime.dataXHR = webview.ajaxData();
      ...
      class WebView {
        ...
        applyData(templ) {
          ...
          let rawData = this.render = this.render || {},
            hookData,
            title = this.lang && this.lang.title ? this.lang.title : cache[this.path]._title || self.define('PROJECT_NAME');

          // 如果是一个被恢复且非监听的webview，则直接显示此webview
          if (this.prop.status === 'restore') return WebApp.transition(this, title);

          // 合并得到渲染数据
          if (!isNull(this.route.render)) hookData = this.route.render(rawData, this, self);

          // 渲染模板
          this.$webview.append(this.compile(templ || this.templ[0], hookData ? $.extend(rawData, hookData) : rawData, title, this.lang));
      ...
      const GC = new class {
        constructor() {
          ...
          setInterval(GCInterval, interval);
        }
        push(webview) {
